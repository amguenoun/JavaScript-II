...
//  updates to their facilities.  Each business has assigned a representative to attend 
//  the event along with a small donation.
...
fullName = runners.map((item) => { return { fName: item.first_name, lName: item.last_name } });
...
allCaps = fullName.map((item) => item.fName.toUpperCase());
console.log(allCaps);
...
// Problem 1
// We need to contact the runners. Make an array of their emails

const runnerEmails = runners.map((item) => item.email);
console.log(runnerEmails);

// Problem 2
//We need the total donations of those that donated above 100
const donationHundred = runners.reduce((acc, item) => {
